...
class TestChorusLapilli(unittest.TestCase):
    ...
    REACT_HOST_ADDR = 'http://localhost:5173'
...
# =========================== [ADD YOUR TESTS HERE] ===========================
    ...
    def test_winner(self):
        '''Check if the game declares a winner, and then stops accepting moves.'''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
    ...
    # X goes first
        ...
        tiles[2].click()  # X in top-right => X should win here

    # Confirm X in positions 0,1,2; O in positions 3,4
        self.assertTileIs(tiles[0], self.SYMBOL_X)
    ...
    # Now, a winning state is reached.  
        tiles[5].click()
    # because the game should be over.
        self.assertTileIs(tiles[5], self.SYMBOL_BLANK)

    def test_cannot_place_4th_piece(self):
        '''Check if the user cannot place a 4th piece.'''
        tiles = self.driver.find_elements(By.XPATH, self.BOARD_TILE_XPATH)
        tiles[0].click()  # X in top-left
        tiles[1].click()  # O in top-middle
        tiles[2].click()  # X in top-right
        tiles[3].click()  # O in middle-left
        tiles[4].click()  # X in center
        tiles[5].click()  # O in middle-right

        tiles[6].click()  # X in bottom-left
    ...
    def test_move_after_3(self):
        ...
        tiles[5].click()  # O in middle-right
        #X is next turn
        #move center to center bottom 
        #ake sure empty first
        self.assertTileIs(tiles[7], self.SYMBOL_BLANK)
        tiles[4].click()  # X in center
        tiles[7].click()  # O in bottom-middle
